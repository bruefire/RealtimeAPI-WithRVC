...
if __name__ == "__main__":
    ...
    class GUI:
        ...
        def load(self):
            ...
            try:
                with open("configs/config_vc.json", "r") as j:
                    data = json.load(j)
                ...
                with open("configs/config_vc.json", "w") as j:
                    data = {
                        "pth_path": " ",
                        "index_path": " ",
        ...
        def launcher(self):
            ...
            layout = [
                [
                    sg.Frame(
                        ...
                        layout=[
                            [
                                sg.Text(i18n("API Key")),
                                sg.Input(
                                    default_text=data.get("openai_api_key", ""),
                                    key="openai_api_key",
                                )
                            ],
                            [
                                sg.Text(i18n("Initial Prompt")),
                                sg.Multiline(
                                    default_text=data.get("openai_initial_prompt", ""),
                                    size=(60, 4), 
                                    key="openai_initial_prompt",
                                )
                            ]
                        ]
                    )
            ...
            self.window = sg.Window("RVC - Voice Chat", layout=layout, finalize=True)
            self.event_handler()
                ...
                if event == "start_vc" and self.flag_vc == False:
                    if self.set_values(values) == True:
                        ...
                        settings = {
                            ...
                            "openai_initial_prompt": values["openai_initial_prompt"],
                        }
                        with open("configs/config_vc.json", "w") as j:
        ...
        def set_values(self, values):
            ...
            self.gui_config.openai_api_key = values["openai_api_key"]
            self.gui_config.openai_initial_prompt = values["openai_initial_prompt"]
        ...
        def start_vc(self):
            ...
            #asyncio.get_event_loop().create_task(self.soundinput())
        ...
        def soundinput(self):
            ...
            vc = avc.AiVoiceChat(
                self.chat_callback,
                self.gui_config.openai_api_key,
                self.gui_config.openai_initial_prompt)
            
            self.tmp_audio_delta = np.array([], np.float32)
            self.audio_delta = np.array([], np.float32)

            paudio = pyaudio.PyAudio()
            stream = paudio.open(
                format=self.FORMAT,
                channels=self.CHANNELS,
                rate=self.gui_config.samplerate,
                output=True,
                frames_per_buffer=self.O_CHUNK,
                stream_callback=self.pa_callback  # コールバック関数を設定
            )
            stream.start_stream()

            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            loop.create_task(vc.start())
            
            async def monitor():
                global stream_latency
                stream_latency = 0.1
                while self.flag_vc and vc.is_disposed() == False:
                    stream_latency = 0.1
                    await asyncio.sleep(0.01)
                
                self.flag_vc = False
                vc.dispose()
                while vc.is_disposed() == False:
                    await asyncio.sleep(0.01)
                loop.stop()
        ...
        def chat_callback(self, base64data):
            
            if base64data == None:
                self.deal_last_response()
            else:
                self.deal_response(base64data)


        def deal_response(self, base64data):
            float_data = np.frombuffer(base64.b64decode(base64data), dtype=np.int16).astype(np.float32) / 32768.0
            tmp_audio_delta = librosa.resample(float_data, orig_sr=self.RATE, target_sr=self.gui_config.samplerate)
            tmp_audio_delta = np.concatenate((self.tmp_audio_delta, tmp_audio_delta), axis=0)

            if tmp_audio_delta.size >= self.block_frame:
                self.tmp_audio_delta = tmp_audio_delta[self.block_frame:]
                indata = [tmp_audio_delta[:self.block_frame], tmp_audio_delta[:self.block_frame]]
                indata = np.swapaxes(indata, 0, 1)
                
                # `outdata` 用のバッファを準備
                outdata = np.zeros_like(indata, dtype=np.float32)
                # audio_callback を呼び出して処理
                self.audio_callback(indata, outdata, None, None, None)
                outdata = np.swapaxes(outdata, 0, 1)
                self.audio_delta = np.concatenate((self.audio_delta, outdata[0]), axis=0)

            else:
                self.tmp_audio_delta = tmp_audio_delta


        def deal_last_response(self):
            tmp_audio_delta = self.tmp_audio_delta
            block_size = self.block_frame

            if tmp_audio_delta.size % block_size != 0:
                tmp_audio_delta = np.pad(tmp_audio_delta, (0, tmp_audio_delta.size % block_size), mode="constant")

            for i in range(tmp_audio_delta.size // block_size):
                indata = [
                    tmp_audio_delta[block_size * i : block_size * (i + 1)], 
                    tmp_audio_delta[block_size * i : block_size * (i + 1)]]
                indata = np.swapaxes(indata, 0, 1)
        ...
        def pa_callback(self, in_data, frame_count, time_info, status):
            frame_data = None
            required_count = self.O_CHUNK
            
            audio_data = self.audio_delta

            # 最後の部分でデータが足りない場合は余白をゼロで埋める
            if audio_data.size < required_count:
                self.audio_delta = np.array([], np.float32)
                frame_data = np.pad(audio_data, (0, required_count - audio_data.size), mode='constant')
            else:
                self.audio_delta = audio_data[required_count:]
                frame_data = audio_data[0:required_count]

            return (frame_data, pyaudio.paContinue)


        # def chat_callback(self, base64_audios):
        #     # Base64のデコード
        #     audio_array = [base64.b64decode(base64_audio) for base64_audio in base64_audios]
        #     audio_data = b''.join(audio_array)

        #     # PCM16 (int16) データを numpy 配列に変換
        #     audio_np = np.frombuffer(audio_data, dtype=np.int16)

        #     # float32に変換（-1.0から1.0の範囲）
        #     audio_np = audio_np.astype(np.float32) / 32768.0

        #     RATE = 24000
        #     CHANNELS = 1
        #     CHUNK = self.block_frame
        #     with sd.OutputStream(samplerate=RATE, channels=CHANNELS, dtype='float32') as stream:
        #         num_chunks = len(audio_np) // CHUNK
        #         for i in range(num_chunks):
        #             # フレームデータを取得
        #             frame_data = audio_np[i * CHUNK: (i + 1) * CHUNK]

        #             # `outdata` 用のバッファを準備
        #             outdata = np.zeros_like(frame_data, dtype=np.float32)

        #             # audio_callback を呼び出して処理
        #             self.audio_callback(frame_data, outdata, CHUNK, None, None)
